function printData() {
    var getHeaderRow = () => {
        var headers = Array.from(document.querySelector("#TraderTable tr").querySelectorAll('th'));
        if (headers.length > 0) {
            headers.pop();
            headers.shift();
        }
        ...
        return `<tr>${headers.map(cell => cell.outerHTML).join('')}</tr>`;
    }

    var payments = Array.from(document.querySelectorAll("#TraderTable tr"))
        .map((element, index) => {
