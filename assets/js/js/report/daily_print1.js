function printData() {
    ...
    var balances = Array.from(document.querySelectorAll("#total h2"))
    .map(element => element.textContent)
    .join(' - '); // استخدم فاصلة أو أي رمز آخر هنا للفصل بين البيانات.

    var payments = Array.from(document.querySelectorAll("#kt_table_company_report tr"))
    .map(element => {
        var cloneElement = element.cloneNode(true); // انسخ العنصر لتجنب التأثير على الجدول الأصلي
        cloneElement.removeChild(cloneElement.cells[0]); // إزالة العمود الأول
        return cloneElement.outerHTML; // إعادة القيمة بعد التعديل
    });

    var paymentsHTML = '';
    // Fetch table headers
    var headersElement = document.querySelector("#kt_table_company_report tr").cloneNode(true);
    headersElement.removeChild(headersElement.cells[0]);
    var tableHeaders = headersElement.outerHTML;
    
    var paymentsPerPageFirstPage = 19;  // How many payments you want per the first page
    var paymentsPerPageOtherPages = 21;  // How many payments you want per other pages
